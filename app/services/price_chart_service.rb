class PriceChartService
  ...
  def get_flip_count
    ...
    records.group_by{|r| r.sold_time.to_date}.sort_by{|date, records| date}.each do |date, records|
      result.merge!({date => {total_count: records.size, successful_count: records.count{|n| n.roi_usd.to_f > 0 || n.same_coin? && n.roi.to_f > 0}, failed_count: records.count{|n| n.roi_usd.to_f < 0 || n.same_coin? && n.roi.to_f < 0}, date: date}})
    end
    result
  end
end